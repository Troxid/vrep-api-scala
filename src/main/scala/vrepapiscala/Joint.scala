...
import coppelia.{FloatWA, FloatW, remoteApi}

/**
  * Created by troxid on 22.11.15.
  */
class Joint private[vrepapiscala](remote: remoteApi, id: Int, handle: Int) {
  ...
  def setTargetVelocity(target: Float): Unit = {
    remote.simxSetJointTargetVelocity(id, handle, target, OpMode.OneShot.rawCode)
  }
  ...
  def setTargetPosition(target: Float): Unit = {
    remote.simxSetJointTargetPosition(id, handle, target, OpMode.OneShot.rawCode)
  }
  ...
  def getForce: Float = {
    ...
    remote.simxGetJointForce(id, handle, h, OpMode.OneShot.rawCode)
    h.getValue
  }
  ...
  /** Sets the maximum force or torque that a joint can exert.
    ...
    */
  def setForce(force: Float): Unit = {
    remote.simxSetJointForce(id, handle, force, OpMode.OneShot.rawCode)
  }
  ...
  def setPosition(position: Float): Unit = {
    remote.simxSetJointPosition(id, handle, position, OpMode.OneShot.rawCode)
  }
  ...
  def getPosition: Float ={
    val h = new FloatW(-1)
    //TODO: simx_opmode_streaming (the first call) and simx_opmode_buffer (the following calls)
    remote.simxGetJointPosition(id, handle, h, OpMode.OneShot.rawCode)
    h.getValue
  }
  ...
  def getMatrix: Array[Float] = {
    ...
    //TODO: simx_opmode_streaming (the first call) and simx_opmode_buffer (the following calls)
    remote.simxGetJointMatrix(id, handle, h, OpMode.OneShot.rawCode)
    h.getArray
  }
  ...
  def setSphericalMatrix(matrix: Array[Float]) = {
    ...
    remote.simxSetSphericalJointMatrix(id, handle, h, OpMode.OneShot.rawCode)
  }
