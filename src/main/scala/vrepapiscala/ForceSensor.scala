...
import coppelia.{FloatWA, IntWA, IntW, remoteApi}
...
class ForceSensor private[vrepapiscala](remote: remoteApi, id: Int, handle: Int) {
  ...
  def read: ForceSensor#Values = {
    ...
    remote.simxReadForceSensor(id, handle, state, forceVec, torqueVec, OpMode.OneShot.rawCode)
  ...
  /**
    ...
    */
  def break(): Unit = {
    remote.simxBreakForceSensor(id, handle, OpMode.OneShot.rawCode)
  }
